...
- **Academy + Landing page (new today)**:
...
- Verification: `pnpm typecheck`, `pnpm lint`, `pnpm test:all` (53 app + 98 engine — +4 layoutFromMines tests, +13 replay, +10 markdown) all green. Browser walk still owed across the recent slices — the Playwright profile is held by the user's other Chrome session. Smoke for the next session: open `/` → click each feature pillar → reach `/learn`, click a lesson, complete the practice board (the deduction should win); open `/daily` after submitting a result → click "Share on X", paste the URL into a preview tool, confirm the OG image renders; open `/games/<id>/review` → "Discuss with Coach" → land in `/coach?conversation=<id>` → ask "what did I miss on turn N?" and check the model references the seeded analysis.
...
Academy + landing just shipped. Phase 1 §6 line items remaining:

- **Dark/light theme toggle** — every component uses `dark:` Tailwind classes already; need a persisted toggle (system / light / dark) wired through a top-of-tree provider that respects `prefers-color-scheme` on first paint without flashing. Spec §6.
- **Minimal PostHog events** — `landing_view`, `lesson_view`, `daily_complete`, `share_card_copied`, `coach_message_sent`, `post_game_review_opened`, `pro_modal_view`. Pull forward from the Phase 1.5 list since the events surface user behavior on the demo. Spec §6.
- **Pro tier UX split** — review is currently open to both paid tiers; PROJECT_PLAN.md §14 reserves "full" review for Pro. Optional polish: cap the missed-mistake count or hide the action log for Pro Lite.

CTO recommendation: bundle **theme toggle + PostHog events** as one short polish slice — both are small and finish off Phase 1 §6.
