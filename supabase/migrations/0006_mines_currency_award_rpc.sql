-- Award Mines for a completed game exactly once.
-- Rates mirror ideas/PROJECT_PLAN.md §14:
-- beginner/intermediate/expert wins = 5/15/50, Daily win = 25,
-- losses earn 20% of the corresponding win value.

CREATE UNIQUE INDEX IF NOT EXISTS mines_transactions_finished_game_once_idx
  ON public.mines_transactions (user_id, reason, source_id)
  WHERE source_id IS NOT NULL
    AND reason IN ('game_finish', 'daily_finish');
...
BEGIN
  ...
  FROM public.mines_transactions
  WHERE public.mines_transactions.user_id = caller_id
    AND public.mines_transactions.reason = transaction_reason
    AND public.mines_transactions.source_id = target_game_id;
  ...
  SET
    mines_balance = mines_balance + mine_delta,
    total_earned = total_earned + mine_delta,
    last_earn_at = now(),
    updated_at = now()
  WHERE public.user_currency.user_id = caller_id
  RETURNING *
